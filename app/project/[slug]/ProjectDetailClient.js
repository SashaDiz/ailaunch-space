...
function ProjectDetailPageContent({ initialProject, slug: slugProp }) {
  ...
  useEffect(() => {
    // If we have initial data from server, fetch fresh data for user-specific fields (userVoted, views)
    // If no initial data, do a full fetch
  ...
  const fetchProject = async () => {
    ...
    try {
      ...
      // Only show loading spinner if we don't have initial data
      ...
      if (response.ok) {
        ...
        if (!initialProject) setProject(null);
        toast.error("AI project not found");
      } else {
        if (!initialProject) toast.error("Failed to load AI project");
      }
    } catch (error) {
...
export default function ProjectDetailClient({ initialProject, slug }) {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-base-100">
    ...
    >
      <ProjectDetailPageContent initialProject={initialProject} slug={slug} />
    </Suspense>
  );
}
