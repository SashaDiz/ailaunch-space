...
export default function LoadingSpinner({ 
  ...
  const spinner = (
    <div 
      className={`inline-flex items-center justify-center ${sizeClasses[size]} border-4 border-border border-t-black rounded-full animate-spin`}
      role="status"
      aria-label="Loading"
    >
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          {spinner}
          {message && (
            <p className="text-muted-foreground font-medium mt-4">{message}</p>
          )}
        </div>
      </div>
    );
