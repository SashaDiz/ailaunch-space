...
export default function UserProfileLink({
  ...
  return (
    <Link 
      href={`/user/${userId}`}
      className={`inline-flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors ${sizeClasses[size]} ${className}`}
    >
      {showAvatar && (
        <div className={`${avatarSizes[size].className} rounded-full overflow-hidden flex-shrink-0`}>
          {userAvatar && !avatarError ? (
            ...
            <div className="bg-foreground text-background w-full h-full flex items-center justify-center text-xs font-bold">
