...
export class SupabaseDatabaseManager {
  ...
  getClient() {
    ...
    if (!this.client) {
      ...
      throw new Error('Database client not initialized. Please check NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SECRET_KEY (or legacy SUPABASE_SERVICE_ROLE_KEY) environment variables.');
    }
  ...
  applyFilters(query: any, filter: Record<string, any>) {
      ...
      if (key === '$and') {
        ...
        try {
          const orConditions = value.map(condition => {
            ...
            if (entries.length === 1) {
              const [field, val] = entries[0] as [string, any];
              ...
              // Handle boolean values correctly for single field
              const singleValue = val === true ? 'true' : val === false ? 'false' : val;
              return `${field}.eq.${singleValue}`;
            } else if (entries.length > 1) {
              // Handle multiple fields in one OR condition (AND within OR)
              // Format: "field1.eq.value1,field2.eq.value2" (comma means AND within this OR group)
              const conditions = entries.map(([field, val]: [string, any]) => {
                if (typeof val === 'object' && val !== null) {
                  if (val.$ne !== undefined) {
                    const neValue = val.$ne === true ? 'true' : val.$ne === false ? 'false' : val.$ne;
                    return `${field}.neq.${neValue}`;
                  } else if (val.$exists !== undefined) {
                    return val.$exists ? `${field}.not.is.null` : `${field}.is.null`;
                  } else if (val.$gt !== undefined) {
                    const compareValue = val.$gt instanceof Date ? val.$gt.toISOString() : val.$gt;
                ...
                // Default to equality - handle boolean values correctly
          ...
          if (orConditions.length > 0) {
            // Join OR conditions - each condition is a separate OR group
            // Format: "condition1,condition2" where condition1 and condition2 are OR'd together
            // If a condition has commas inside (multiple fields), those are AND'd within that OR group
        ...
        // Handle null values properly - use .is() instead of .eq()
        if (value === null) {
          query = query.is(key, null);
        } else {
          // Simple equality - convert Date objects to ISO strings
          // For UUID fields, ensure proper string comparison
          let compareValue = value instanceof Date ? value.toISOString() : value;
          
          // Ensure UUID fields are compared as strings
          if (key === 'submitted_by' || key === 'id' || key.endsWith('_id') || key === 'user_id' || key === 'app_id') {
  ...
  // FIND ONE
  async findOne(collectionName, query: Record<string, any> = {}, options: Record<string, any> = {}) {
    const table = this.getTableName(collectionName);
    const client = this.getClient();
  ...
  // FIND
  async find(collectionName, query: Record<string, any> = {}, options: Record<string, any> = {}) {
    const table = this.getTableName(collectionName);
    const client = this.getClient();
  ...
  // UPDATE MANY
  async updateMany(collectionName, filter, update, options: Record<string, any> = {}) {
  ...
  // COUNT
  async count(collectionName, query: Record<string, any> = {}) {
    const table = this.getTableName(collectionName);
    const client = this.getClient();
  ...
  async createIndex(collectionName, indexSpec, options: Record<string, any> = {}) {
