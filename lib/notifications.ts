...
class NotificationManager {
  ...
  async sendNotification({
    ...
    try {
      ...
      // These are critical notifications that users should always receive:
      // - Account-related: creation, deletion
  ...
  async getUserNotificationPreferences(userId) {
    try {
      // If userId is null, return default preferences (for newsletter-style notifications)
      if (!userId) {
   ...
   * Note: Mandatory notifications are always sent regardless of preference
   */
  getDefaultPreferences() {
    return {
      ...
      launch_week_reminder: true,    // Mandatory - users in launch week must receive
   ...
   * @param {Object} params - Parameters
   */
  async sendSubmissionApprovalNotification({ userId, userEmail, project, competition, projectStatus }) {
    const normalizedStatus = projectStatus || project.status || 'live';
    const competitionStartDate = competition?.start_date
  ...
  async sendCompetitionWinnerNotification({ userId, userEmail, project, competition, position }) {
    return await this.sendNotification({
      ...
      data: {
        ...
        planType: project.plan_type || 'standard'
      }
    });
  }
   ...
   */
  async getUserNotificationHistory(userId: string, options: Record<string, any> = {}) {
    try {
